...
import os
from BasicEventEnvironment import Process_Environment
from BasicEventProcessor import MyProc
...
# for MEMORY_LENGTH in []:#,6,8# 2,3,4,5,6,7,8,10,20
MEMORY_LENGTH = opt.memlen
print("pwd",os.getcwd())
data_directory = "../../dataset/"+dataset_name+"/"
result_directory = "../../dataset/"+dataset_name+"/result/"+str(MEMORY_LENGTH)+"/event/"
...
#bins_ranges = processor.get_bin_ranges()
env = Process_Environment(testing = False,training_data = True ,data_directory=data_directory,result_directory=result_directory , window_size = MEMORY_LENGTH, processed_acts_num = nb_events)

#test_env = Process_Environment(testing = True, training_data = True ,data_directory=data_directory,result_directory=result_directory , window_size = MEMORY_LENGTH) #, processed_acts_num = nb_events)

test_data = Process_Environment(testing = True, training_data = False,data_directory=data_directory,result_directory=result_directory , window_size = MEMORY_LENGTH, processed_acts_num = nb_events)
...
MyModel.add(Dense(units=nb_events,activation='linear'))
MyModel.summary()
memory = SequentialMemory(limit=1000000, window_length=MEMORY_LENGTH)#PrioritizedMemory(limit=200000, alpha=.6, start_beta=.2, end_beta=9., steps_annealed=20000, window_length=MEMORY_LENGTH)

dqn = DQNAgent(model=MyModel, nb_actions=nb_events, policy=BoltzmannQPolicy(clip=(-15.,15)), test_policy=GreedyQPolicy() , memory=memory,
...
# dqn.load_weights(result_directory+'DQN_event.h5f')
...
for trance in [1,2,3,4,5,6,7]:
    ...
    dqn.save_weights(result_directory+'DQN_event'+str(trance)+'.h5f', overwrite=True)
