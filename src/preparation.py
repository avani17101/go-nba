...
class Input:
    """
    Data preprocessing module for DL model
    """
    ...
    dcr_path = '' # the part of dcr xml file
    ...
    test_loader = ''
    validation_loader = ''
    design_mat_path = '' #Path to design matrix
    ...
    #start from here
    @classmethod
    def run(cls,path, dcr_path, design_mat_path, prefix, batch_size, mode = "event_timestamp_prediction"):
        '''
        ...
        cls.mode = mode
        cls.path = os.getcwd() + "/checkpoints/" + cls.dataset_name + '/'+mode + '/prefix_' + str(cls.prefix_len)
        cls.dcr_path = dcr_path

        
        #Creating a design matrix that shows one hot vector representation for activity IDs
        if(design_mat_path): #design mat is same for particular dataset, hence to speed up we can precompute it and pass path
            cls.design_matrix = pd.read_pickle(design_mat_path)
            ...
            if(path.split('.')[1] == 'csv'):
                ...
                data_augment.to_pickle("dataset/preprocessed/"+cls.dataset_name+".pkl")
                print(data_augment)
                # return 
            ...
            cls.design_matrix, design_mat_path = cls.__design_matrix_creation(data_augment)
            """ uncomment below for design mat pre computation"""
            # return design_mat_path  
    ...
    def __read_csv_massive(cls, path):
        ...
        # print("saved preprocessed at :"+str(name )+ ".pkl")
        return results
    ...
    def __design_matrix_creation(cls, data_augment):
        ...
        file = "dataset/preprocessed/"+cls.dataset_name+"_design_mat.pkl"
        pickle.dump(design_matrix, open(file, "wb"))
        print("saved preprocessed at :", file)
        
        return design_matrix, file
    ################################################################################
    # Creating the desing matrix based on given prefix.
    @classmethod
    def prefix_creating(cls, prefix=2, mode = 'event_prediction'):
        
        unique_event = sorted(cls.design_matrix['class'].unique())
        cls.unique_event = [0] + unique_event

        if (mode == "event_prediction"):
            clsN = cls.design_matrix.columns.get_loc('class')
        elif (mode == 'event_timestamp_prediction' or "timestamp_prediction"):
...
#             gr = gr.drop('CaseID', axis=1)
            ...
            if(mode ==  "timestamp_prediction"):
                if (gr.shape[0] - 1 > prefix):
                    for i in range(gr.shape[0]):
                        ...
                        gr_cp = gr.copy()
                        gr_cp.iloc[i + prefix:i + prefix+1,gr_cp.columns.get_loc('duration_time')] = -1
                        tens = torch.tensor(gr_cp.iloc[i:i + prefix+1].values, dtype=torch.float, requires_grad=False)
                        temp.append(tens)
            else:
                if (gr.shape[0] - 1 > prefix):
                    for i in range(gr.shape[0]):
        ...
        print("cls.design_matrix_padded cls.y \n",cls.design_matrix_padded, cls.y)
    ...
    def train_valid_test_index(cls):
        ...
        print("size", cls.design_matrix_padded.size()[0] * .8)
