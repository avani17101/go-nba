from rl_base_tf_action_mask import Env_base
...
parser.add_argument("--env_name", default="traffic_ss", help="dataset name")
parser.add_argument("--agent_type", default="ppo", help="agent type")
parser.add_argument("--train_epi", default=100, type = int, help="train episodes")
parser.add_argument("--test_epi", default=20, type = int, help="test episodes")
parser.add_argument("--reward_type", help="reward type")
...
if agent_type == 'ppo':
    agent = Agent.create(
        agent=agent_type, environment=environment, batch_size=32, learning_rate=1e-3,config = {"device":'GPU'}
    ) 
if agent_type == 'dqn':
    agent = Agent.create(
        agent='dqn', environment=environment, batch_size=32, memory=900000,learning_rate=1e-1,config = {"device":'GPU'}
    )

if agent_type == 'double_dqn':
    agent = Agent.create(
        agent='double_dqn', environment=environment, batch_size=32, learning_rate=1e-3,config = {"device":'GPU'}
    )
path = "/ssd_scratch/cvit/avani.gupta/"
save_path = path+"tensorforce_logs/"+agent_type+env_name+reward_type+"mask"
if not os.path.isdir(save_path):
    os.makedirs(save_path)
log_path = "tensorforce_logs/"+agent_type+env_name+reward_type+"mask"+"train.csv"
...
# testing agent
log_path = "tensorforce_logs/"+agent_type+env_name+reward_type+"mask_test.csv"
environment = Environment.create(
