...
def get_obs_bounds2(df2):
    low_array = np.zeros(len(df2.columns)-3)
    # low_array[-2] = min(df2['class'])
    # low_array[-1] = min(df2['CaseID'])
    high_array = np.ones(len(df2.columns)-3)
    high_array[-1] = max(df2['duration_time'])
    # high_array[-3] = max(df2['remaining_time'])
    # high_array[-2] = max(df2['class'])
    # high_array[-1] = max(df2['CaseID'])
    return low_array, high_array

def get_maes(env, prefix_e):
    prefix_s = 1
    res = []
    for i in range(prefix_s, prefix_e):
        path = 'checkpoints/'+env+'/timestamp_prediction/prefix_'+str(i)
        with open(path + '/results2.txt', "r") as fout:
            res.append(float(fout.read()[:8]))
    return np.array(res)
